...
interface OperationalData {
  healthFacilities:   number; // % functional
  displacementAlerts: number;
  hazardLevel:        number; // 1–5
  alertText:          string;
}
...
interface TrendPoint  { month: string; underfunding: number } // 0–100
...
export interface DossierProps {
  ...
  state?:  string; // optional admin-1 region (triggers state-level dossier)
  onClose: () => void;
}
...
function getCached(key: string): DossierData | null {
  try {
    const raw = localStorage.getItem(`dossier_${key}`);
    if (!raw) return null;
...
function setCached(key: string, data: DossierData) {
  try {
    localStorage.setItem(`dossier_${key}`, JSON.stringify({ data, ts: Date.now() }));
...
For ${location} as of February 2026, return this exact JSON:
{
  ...
  "operational": {
    "healthFacilities":   <integer 0-100 % still functional>,
    "displacementAlerts": <integer count of active alerts>,
    "hazardLevel":        <integer 1-5>,
    "alertText":          "<one sentence describing the most urgent field alert>"
  },
  "sectors": {
    "food":   { "gapScore": <0.0-1.0 float>, "label": "<short status>" },
    "water":  { "gapScore": <0.0-1.0 float>, "label": "<short status>" },
    "health": { "gapScore": <0.0-1.0 float>, "label": "<short status>" }
  },
  "trend": [
  ...
  "articles": [
    {
      "title":      "<specific, bold news headline>",
      "summary":    "<2 sentence factual summary>",
      "source":     <"Reuters"|"NYT"|"WSJ"|"BBC"|"AP">,
      "url":        "<realistic full URL from that source, e.g. https://www.reuters.com/world/...>",
      "imageQuery": "<3-word Unsplash search phrase>"
    }
  ]
}

Include exactly 3 articles on the most urgent developments. The trend array should show realistic 6-month underfunding escalation. Be specific and factual.`;
}
...
function wmoToCondition(code: number): WeatherData["condition"] {
  ...
  if (code <= 77 || code <= 86) return "Snow";
  return "Storm";
}
...
function SunIcon()  {
  return <svg {...ip} className="text-amber-300"><circle cx="12" cy="12" r="4" />{[[12,2,12,5],[12,19,12,22],[2,12,5,12],[19,12,22,12],[4.22,4.22,6.34,6.34],[17.66,17.66,19.78,19.78],[4.22,19.78,6.34,17.66],[17.66,6.34,19.78,4.22]].map(([x1,y1,x2,y2],i)=><line key={i} x1={x1} y1={y1} x2={x2} y2={y2}/>)}</svg>;
}
function CloudIcon(){
  return <svg {...ip} className="text-slate-300"><path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9z"/></svg>;
}
function RainIcon(){
  return <svg {...ip} className="text-sky-400"><path d="M20 17.58A5 5 0 0 0 18 8h-1.26A8 8 0 1 0 4 16.25"/><line x1="8" y1="19" x2="8" y2="21"/><line x1="8" y1="13" x2="8" y2="15"/><line x1="16" y1="19" x2="16" y2="21"/><line x1="16" y1="13" x2="16" y2="15"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="12" y1="15" x2="12" y2="17"/></svg>;
}
function StormIcon(){
  return <svg {...ip} className="text-yellow-300"><path d="M19 16.9A5 5 0 0 0 18 7h-1.26a8 8 0 1 0-11.62 9"/><polyline points="13 11 9 17 15 17 11 23"/></svg>;
}
function HazeIcon(){
  return <svg {...ip} className="text-slate-400"><path d="M5 5h3m4 0h9M3 10h11m4 0h1M1 15h11m4 0h7M3 20h5m6 0h11" strokeWidth="1.8"/></svg>;
}
function SnowIcon(){
  return <svg {...ip} className="text-sky-200"><line x1="12" y1="2" x2="12" y2="22"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="4.93" y1="19.07" x2="19.07" y2="4.93"/></svg>;
}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <span className="text-[10px] font-bold font-mono"
                  style={{ color: unit === "C" ? "rgba(34,211,238,0.9)" : "rgba(255,255,255,0.22)" }}>°C</span>
            <span className="text-white/12 text-[10px] mx-0.5">|</span>
            <span className="text-[10px] font-bold font-mono"
                  style={{ color: unit === "F" ? "rgba(34,211,238,0.9)" : "rgba(255,255,255,0.22)" }}>°F</span>
          </button>
          <div className="flex items-center gap-1">
            <div className={`w-1 h-1 rounded-full ${isLive ? "bg-emerald-400 animate-pulse" : "bg-white/15"}`} />
            <span className={`text-[9px] font-mono ${isLive ? "text-emerald-400/60" : "text-white/18"}`}>
              {isLive ? "LIVE" : "EST"}
            </span>
          </div>
        </div>
...
function SectorStatus({ sectors }: { sectors: SectorData }) {
  ...
  return (
    <div>
      <p className="text-[9px] text-white/22 uppercase tracking-[0.2em] font-mono font-semibold mb-2.5">
        Sector Gap Analysis
      </p>
          ...
          return (
            <div
              ...
              style={{
                ...
                borderColor: critical ? "rgba(248,113,113,0.20)" : "rgba(255,255,255,0.06)",
              }}
            >
              {/* top severity bar */}
              ...
              <div className="px-2.5 py-3 flex flex-col gap-0.5">
                ...
                <p className="text-[8px] font-mono font-bold mt-1.5 uppercase tracking-wide" style={{ color: accent }}>
                  {status}
                </p>
      ...
      <div className="px-2 pt-2 pb-3">
        <ResponsiveContainer width="100%" height={90}>
          <LineChart data={trend} margin={{ top: 6, right: 8, bottom: 0, left: 0 }}>
            <XAxis
              dataKey="month"
              tick={{ fill: "rgba(255,255,255,0.22)", fontSize: 9, fontFamily: "monospace" }}
            ...
            <Line
              type="monotone"
              dataKey="underfunding"
              stroke={critical ? "#f87171" : "#34d399"}
              strokeWidth={2}
...
// ─── Intelligence card (document brief style) ──────────────────────────────
function IntelligenceCard({ article, index }: { article: Article; index: number }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.55 + index * 0.12, duration: 0.38, ease: "easeOut" }}
      onClick={open}
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-2 border-b border-white/[0.05]">
        <div className="flex items-center gap-2">
          <span className="text-[8px] font-mono font-bold text-white/20 tabular-nums">
        ...
        <span className="text-white/18 text-[9px] font-mono opacity-0 group-hover:opacity-100 transition-opacity">
          ↗ open
        </span>
      </div>
      ...
      {/* headline + summary */}
      <div className="px-4 py-3">
        <h3 className="text-white/90 font-semibold text-[12px] leading-snug tracking-tight mb-1.5
...
function Skeleton() {
  return (
    <div className="space-y-3">
      {/* weather skeleton */}
      <div className="rounded-2xl border border-white/6 overflow-hidden" style={{ background: "rgba(15,20,40,0.55)" }}>
        <div className="px-4 pt-3 pb-2 border-b border-white/6">
          <div className="h-2 w-32 rounded bg-white/8 animate-pulse" />
        </div>
      ...
      <div className="rounded-2xl border border-white/6 overflow-hidden" style={{ background: "rgba(15,20,40,0.55)" }}>
        <div className="px-4 pt-3 pb-2 border-b border-white/6"><div className="h-2 w-24 rounded bg-white/8 animate-pulse" /></div>
        <div className="px-4 py-3 space-y-3">{[...Array(3)].map((_,i)=><div key={i} className="h-5 rounded bg-white/6 animate-pulse" style={{animationDelay:`${i*80}ms`}}/>)}</div>
      </div>
      {/* sector skeleton */}
      <div className="grid grid-cols-3 gap-2">
        {[...Array(3)].map((_,i)=><div key={i} className="rounded-xl border border-white/6 h-20 animate-pulse" style={{background:"rgba(15,20,40,0.55)",animationDelay:`${i*80}ms`}}/>)}
      </div>
      {/* trend skeleton */}
      <div className="rounded-2xl border border-white/6 h-28 animate-pulse" style={{ background: "rgba(15,20,40,0.55)" }} />
      {/* card skeletons */}
      {[...Array(3)].map((_,i)=>(
        <div key={i} className="rounded-2xl border border-white/6 overflow-hidden animate-pulse" style={{background:"rgba(12,16,35,0.50)",animationDelay:`${i*100}ms`}}>
...
export function IntelligenceDossier({ country, crisis, state, onClose }: DossierProps) {
  ...
  useEffect(() => {
    ...
    // Groq — operational / sector / trend / articles
    fetchDossier(country, state)
      .then(d => { if (!cancelled) setData(d); })
      .catch(e => { if (!cancelled) setError(e.message ?? "Failed to load"); })
      .finally(() => { if (!cancelled) setLoading(false); });

    // Open-Meteo — real-time weather (runs in parallel, doesn't block)
  ...
  // Prefer real Open-Meteo data; fall back to Groq estimate
  ...
  return (
    <motion.div
      ...
      className="fixed top-0 right-0 h-screen w-[380px] z-50 flex flex-col
    ...
    >
      ...
      <div className="shrink-0 px-6 pt-5 pb-4 border-b border-white/[0.06]">
        <div className="flex items-start justify-between mb-3">
          ...
          <button
            onClick={onClose}
            className="w-6 h-6 flex items-center justify-center rounded text-white/25
                       hover:text-white hover:bg-white/[0.06] transition-all text-sm"
          >✕</button>
        </div>
        <div>
          ...
          {state && (
            <p className="text-white/35 text-[11px] font-mono mt-0.5 uppercase tracking-wider">{country}</p>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <motion.p key="err" className="text-red-400/70 text-xs" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ⚠ {error}
            </motion.p>
          ) : data ? (
            <motion.div key="data" className="space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>

              <Reveal delay={0.0}><WeatherTactical w={weatherToShow ?? data.weather} country={state ?? country} isLive={realWeather !== null} /></Reveal>
              <Reveal delay={0.1}><OperationalPulse op={data.operational} /></Reveal>
              <Reveal delay={0.2}><SectorStatus sectors={data.sectors} /></Reveal>
              <Reveal delay={0.3}><TrendGraph trend={data.trend} /></Reveal>

              <Reveal delay={0.4}>
                <div className="flex items-center gap-2 pt-1 border-t border-white/[0.05]">
                  <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "rgba(34,211,238,0.7)" }} />
                  <p className="text-[9px] text-white/22 uppercase tracking-[0.2em] font-mono font-semibold">
                    SIGINT BRIEFS
                  </p>
                </div>
              </Reveal>
      ...
      <div className="shrink-0 px-5 py-3 border-t border-white/6">
        <p className="text-white/12 text-[9px] font-mono tracking-wide">
          Weather: Open-Meteo · Intel: Groq Llama-3.3-70b · {new Date().toUTCString().slice(0, 16)}
        </p>
      </div>
    </motion.div>
  );
